...
static NTSTATUS DdimonpHandleObRegisterCallbacks(
	POB_CALLBACK_REGISTRATION CallbackRegistration,
	PVOID                     *RegistrationHandle);

NTSTATUS DdimonpObReferenceObjectByHandleWithTag(
	HANDLE                     Handle,
...
static ShadowHookTarget g_ddimonp_hook_targets[] = {
	...
	{
		RTL_CONSTANT_STRING(L"OBREFERENCEOBJECTBYHANDLEWITHTAG"),//ObRegisterCallbacks
		DdimonpObReferenceObjectByHandleWithTag,
		nullptr,
	},
};
...
_Use_decl_annotations_ static NTSTATUS  DdimonpHandleObRegisterCallbacks(
	POB_CALLBACK_REGISTRATION CallbackRegistration,
	PVOID                     *RegistrationHandle) {
	const auto original = DdimonpFindOrignal(DdimonpHandleObRegisterCallbacks);
	const auto result = original(CallbackRegistration, RegistrationHandle);
	auto return_addr = _ReturnAddress();
	HYPERPLATFORM_LOG_INFO_SAFE("ObRegisterCallbacks,%08x",return_addr);
	return result;
}
...
extern "C" PEPROCESS GameProcessEPROCESS;
_Use_decl_annotations_ static NTSTATUS DdimonpObReferenceObjectByHandleWithTag(
	HANDLE                     Handle,
	ACCESS_MASK                DesiredAccess,
	POBJECT_TYPE               ObjectType,
	KPROCESSOR_MODE            AccessMode,
	...
	if (memcmp(processName, "windbg.exe", strlen("windbg.exe")) == 0) {
		//DbgPrintEx(101, 0, "DesiredAccess=%llx", DesiredAccess);
		DesiredAccess = 0;
	}
	const auto original = DdimonpFindOrignal(DdimonpObReferenceObjectByHandleWithTag);
	auto result = original(Handle, DesiredAccess, ObjectType, AccessMode, Tag, Object, HandleInformation);
	if (memcmp(processName, "x32dbg.exe", strlen("x32dbg.exe")) == 0)
	{
		DbgPrintEx(101, 0, "1,%llx,%llx,%llx,%llx,%llx,%s\n", result, ObjectType, *(ULONG64*)Object, GameProcessEPROCESS, Handle, PsGetProcessImageFileName((PEPROCESS)*(ULONG64*)Object));
	}
	auto return_addr = _ReturnAddress();
	//   ObjectType == *PsThreadType
	if (ObjectType == *PsProcessType && UserMode== AccessMode&& (INT64)Handle!=(INT64)-1&& result== STATUS_ACCESS_DENIED) {
		//UCHAR processName[500];
		//memset(processName, 0, 500);		
		if (memcmp(processName, "x32dbg.exe", strlen("x32dbg.exe")) == 0) {
			if (GameProcessEPROCESS!=0)
			{
				//ObfReferenceObject(GameProcessEPROCESS);
				//*Object = GameProcessEPROCESS;
				DbgPrintEx(101, 0, "2,%llx,%llx,%llx,%llx,%llx,%s\n", result,  Object, *(ULONG64*)Object, GameProcessEPROCESS, Handle, PsGetProcessImageFileName((PEPROCESS)*(ULONG64*)Object));
				//result = STATUS_SUCCESS;
				//HYPERPLATFORM_LOG_INFO_SAFE("1ObReferenceObjectByHandleWithTag ,%llx", return_addr);
			}else
				HYPERPLATFORM_LOG_INFO_SAFE("2ObReferenceObjectByHandleWithTag ,%llx",  return_addr);
		}
	}	
	return result;
}