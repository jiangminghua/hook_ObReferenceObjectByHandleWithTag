...
_Use_decl_annotations_ static void VmmpHandleMsrAccess(
  ...
  if (read_access) {
    ...
    {
      if (msrarrary[i] == 0) {
        msrarrary[i] = (unsigned int)msr;
        break;
      }
      if (msrarrary[i] == (unsigned int)msr) {
        break;
      }
    }
    if (transfer_to_vmcs) {
      ...
      if ((unsigned int)msr == 0xc0000100 || (unsigned int)msr < 0x1000)
      {
        //msr_value.QuadPart = UtilReadMsr64(msr);
      }
      if ((unsigned int)msr != 0x40000001u)
      {
        if ((unsigned int)msr != 0x2fffu)
        {
          if ((unsigned int)msr != 0xc0002fffu)
          {
            msr_value.QuadPart = UtilReadMsr64(msr);
          }
        }
